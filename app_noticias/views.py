...
class HomePageView(ListView): #ListView realiza uma consulta em algum model.
    model = Noticia #relaciona a view Home com model Noticia. (object_list é váriavel que nos permite acessar a lista de noticias lá no model)
    context_object_name = 'noticias' #estou alterando o nome da object_lista para 'noticias'
    template_name = 'app_noticias/home.html'

    def get_queryset(self): # excluir as nóticias que possui data igual a None e ordenar as noticias em ordem decrecente.
...
class NoticiasResumoView(TemplateView):
    ...
    def get_context_data(self, **kwargs): #**kwargs é um dicionário dinamico que ao chamar a funcção get_context_data() posso chamar passando qlqr parametro.
        context = super().get_context_data(**kwargs) #chama 
